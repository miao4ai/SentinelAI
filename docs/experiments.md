...
如何把融合对比从**合成数据**升级为有说服力的**真实实验**。相关文档：融合分类见 [`fusion.md`](fusion.md)，各层实现见 [`fusion_layers.md`](fusion_layers.md)，首个合成结果见 [`experiment_1.md`](experiment_1.md)。
...
- **报告**：不只报单阈值 P/R——AUC 才反映排序质量（见 experiment_1 里投票基线 F1 差但 AUC 高的教训）。
...
- **做法**：把两模态特征当作各层输入，跑现有 `compare.py` 框架：
  ...
  - ⑤ vote：各自 0/1 → 加权投票
  - ①/② 不适用（无原始信号 / 无 CLIP 对齐）
- **产出**：**experiment_1 的真实版**——真实 2 模态的层级 P/R/AUC 对比 + 对单模态基线的增益（Q1、Q2）。
...
这把 experiment_1 从"合成"升级为"真实"，且成本最低。跑完再决定是否上原始视频做 3 模态（实验 3）。
