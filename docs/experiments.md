...
5. **④ 决策级"活"过来了**：2 模态时它是最差的融合（只有 2 列概率，树学不到东西，§7.1）；3 模态时它**追平 ② coordinated (0.939) 且比它更稳** (std 0.012 vs 0.024)。原因正是**多了一个弱模态就多了可学的东西**——树学会了**给文本降权**，和 ② 同源。**验证了 ④ 的价值随"模态数×质量差异"增长**：模态越多、强弱越不齐，可学习的决策级融合越有意义。原理见 `docs/lecture_10.md`。
...
> 语言无关：XD 是西方电影，用英文 prompt 合适；换中文内容时 `--model OFA-Sys/chinese-clip-vit-base-patch16` + 中文 prompt 即可（`clip_screener.py` 已留接口）。原理见 `docs/lecture_8.md`。
