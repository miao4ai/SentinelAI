...
如何把融合对比从**合成数据**升级为有说服力的**真实实验**。相关文档：融合分类见 [`fusion.md`](fusion.md)，各层实现见 [`fusion_layers.md`](fusion_layers.md)，原理见 [`principle.md`](principle.md)。

---

## 0. 主结果矩阵（所有模型 × 数据集）⭐

一张表看全部结果。行 = 模型（单模态 + 五个融合位置 ①–⑤ + cross-attention ⑥ + VideoMAE 变体），
列 = 数据集/规制。数字 = **F1**（XD：电影分组 5 折 CV mean；UCF：官方 train/test split）。
完整 AUC / std / 方法见下各节。

> **现实约束（决定矩阵能填多满）**：只有 **XD-Violence 是全模态**（视/音/文都自己抽齐了）；
> **UCF-Crime 只有视觉 I3D**（GCS 无音/文/原视频）→ 只能单模态；**Kinetics-400 是 400 类动作、
> 非暴力任务** → 不进此矩阵。所以多模态行的 UCF 列是 **N/A（缺模态）**，不是没做。
...
| *融合位置* | | | |
| ① early（联合 transformer） | **0.951**³ | 0.906 | N/A⁴ |
| ② coordinated（CLIP 式） | 0.936 | 0.939 | N/A⁴ |
| ③ feature-concat | 0.937 | 0.929 | N/A⁴ |
| ④ decision（GBDT stack） | 0.931 | 0.939 | N/A⁴ |
| ⑤ late-fusion（avg） | 0.939 | 0.941 | N/A⁴ |
| ⑥ cross-attn（I3D K/V） | 0.936 | **0.943** | N/A⁴ |
| *⑥ 视频编码器换 VideoMAE* | | | |
| ⑥ VideoMAE-base（自监督） | 0.924 | 0.925 | N/A⁴ |
| ⑥ VideoMAE-k400（监督） | 0.921 | 0.808 | N/A⁴ |

注：① UCF 无音频/文本特征（GCS 只有 I3D）。② XD 2 模态实验不含文本（文本只在 3 模态 774 集抽）。
③ ① 用真实音频 token 序列 0.951（单均值 token 0.945）。④ 融合/cross-attn 需≥2 模态或原始帧，UCF 都没有。
⑤ UCF 是不同任务/协议（弱监督异常检测，官方 train/test split，非电影分组 CV），F1 0.632 偏低但 **AUC 0.885**（见 §10.5）——数值不与 XD 直接可比，仅占"视觉×UCF"这一格。

> **CLIP 零样本初筛**（§9）是不同评测口径（无训练、排序指标）：XD 上 **AUC 0.825、precision@top-10% 0.931**，不进上表。

**跨规制一句话读法**：
- **数据充足（XD 2模态 4502）**：**① early 反超登顶 0.951**（全对称联合 transformer，数据够才发力）。
- **小数据+多模态（XD 3模态 774）**：**⑥ cross-attn 夺魁 0.943**（Q/K/V 结构化偏置省数据）。
- **视频特征换 VideoMAE**：两版都打不过**任务对齐的 I3D**（监督版还更差）。
- **UCF-Crime**：只有视觉 → 单模态基线（异常检测，正常 vs 异常）。
...
### 6.1 补做：把视频侧换成 VideoMAE（诚实的负结果）

上面视频 K/V 用的是 I3D。ROADMAP 6.1 点名要 transformer 时序编码器，于是又抽了一版
...
## 10.5 跨数据集：UCF-Crime 视觉单模态基线

> 日期：2026-09-03 ｜ 脚本：`scripts/exp_ucf_visual.py`

**为什么只有这一行。** UCF-Crime 在 GCS 上**只有 I3D ten-crop 特征**（train 1396 / test 107，
`(T,10,2048)`），**没有音频、文本、原始视频**。所以多模态融合（②③④⑤⑥）、VideoMAE 在 UCF 上
**跑不了**（缺第二个模态 / 缺原始帧）——不是没做，是数据不支持。要在 UCF 上做多模态，得从
UCF 原始来源重下 ~128G 原视频再抽音/文，是另一个大工程。

**做法**：任务 = 正常 vs 异常（文件名 `Normal_*`=正常，其余=异常）。每片段 ten-crop I3D →
crop 均值 → 时序均值 → `(2048)` → StandardScaler + LogisticRegression，用 UCF **官方
train/test split**（非 CV，UCF 每个文件是独立监控视频、无电影分组泄漏问题）。

| 模型 | F1 | P | R | AUC |
|---|---|---|---|---|
| 视觉单模态 (I3D) | 0.632 | 0.562 | 0.720 | **0.885** |

> train 1396（46% 异常）/ test 107（23% 异常），特征维 2048。**F1 0.632 偏低、AUC 0.885 不低**——
> 简化的片段级二分类 + 0.5 固定阈值 + 测试集类别不均（23% 异常）拉低了 F1，但 **AUC 0.885** 说明
> 视觉 I3D 对"正常 vs 异常"的**排序**其实相当好（和 CLIP 零样本一样：排序强、绝对阈值需另调）。

**说明**：这是个**弱监督异常检测**数据集，标准做法是视频级弱标签 + 帧级检测（报 AUC）；这里做的是
**片段级二分类**的简化基线，口径与 XD 的视觉单模态一致，用来在主矩阵里占住"视觉 × UCF"这一格。
不追求 UCF SOTA。
