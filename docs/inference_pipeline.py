...
#
# **Triton 真机搭建实录（L4 box, 2026-09-14）**。以上配置在 GPU 机上实际搭了一遍：
# `docker.io` + `tritonserver:24.08-py3` 镜像，模型仓库即上面的布局，服务正常 READY。
# 踩到两个值得记录的坑：
#
# 1. **opset 版本错位**：新版 skl2onnx 默认导出 opset 22，而 24.08 镜像内的 onnxruntime
#    只支持到 21 → 模型加载失败。修复：`to_onnx(..., target_opset=21)` 重导出。
#    教训：**导出方与部署镜像的 opset 支持范围要对表**，宁可钉死不用默认。
# 2. **tritonclient 的 HTTP 客户端基于 gevent，不可跨线程共享**——并发压测时每个线程
#    必须建自己的 `InferenceServerClient`，否则 `greenlet.error: Cannot switch to a
#    different thread`。
#
# 用 256 条真实测试特征压测（`tritonclient[http]`，loopback）：
#
# | 测量 | 结果 | 解读 |
# |---|---|---|
# | 正确性 | max\|Triton−本地 ort\| = **0.0** | 服务化零精度损失 |
# | batch=1 串行 | mean 1.17 ms, p99 1.45 ms | 比进程内 63µs 多的 ~1.1ms 全是 HTTP/序列化开销 |
# | 并发 1→4→16→64 | 934 → 2172 → 2430 → **2463 请求/秒** | 并发 4 起 dynamic batching + 2 实例已把散请求攒成批；~16 并发吃满平台 |
# | batch=256 单请求 | 59.5 ms（232 µs/clip） | 3.7MB 载荷的传输为主——客户端能自己攒批就别传大批 |
#
# 结论：低耗流的量级坐实——**单机 CPU 上 ~2500 QPS**，964 条测试集 0.4 秒扫完；对照
# 9.1 的 VLM ~256 ms/clip（约 4 QPS），两级速度差 ~600×，这正是级联架构成立的物理基础。
# 生产还差的部分：上游三个编码器（I3D/AST/文本嵌入）才是低耗流的真实耗时大头，应各自做成
# Triton 模型（KIND_GPU）用 ensemble 串联；本 LR 裁决层已经快到可以忽略。
